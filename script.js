...
const TOTAL_LIGHTS = 80;
const TEST_MODE = new URLSearchParams(window.location.search).get("prueba") === "1";
const BASE_STORAGE_KEY = "universoAxelProgressV1";
const STORAGE_KEY = TEST_MODE ? `${BASE_STORAGE_KEY}:prueba` : BASE_STORAGE_KEY;
...
const DUCKED_VOLUME = NORMAL_VOLUME;

const QUALITY_PROFILES = {
  high: { deepStars: 270, motes: 78, bodies: 9, shootMin: 7000, shootMax: 16000 },
  balanced: { deepStars: 190, motes: 44, bodies: 5, shootMin: 13000, shootMax: 27000 },
  saver: { deepStars: 110, motes: 18, bodies: 1, shootMin: 27000, shootMax: 48000 },
};
...
const tracks = [
  ...
  { title: "Summer Eyes", artist: "OHYUL", file: "summer_eyes.mp3", theme: "summer-eyes" },
];
...
try { selectedQuality = localStorage.getItem(QUALITY_KEY) || "auto"; } catch {}
...
const state = {
  ...
  pendingPolarisReveal: false,
  polarisUnlocked: false,
  letterOpened: false,
...
function resolveAutomaticQuality(){
  ...
  if(reduced)return "saver";
  if(mobile&&(memory<=4||cores<=4))return "saver";
  if(mobile||memory<=4||cores<=4)return "balanced";
  return "high";
}
function qualityLabel(value){return {high:"Alta",balanced:"Equilibrada",saver:"Ahorro"}[value]||"Equilibrada"}
function applyQuality(mode,persist=true){
  ...
  if(qualityStatus)qualityStatus.textContent=selectedQuality==="auto"?`Automática · ${qualityLabel(resolvedQuality)}`:qualityLabel(resolvedQuality);
  qualityButtons.forEach(button=>button.setAttribute("aria-pressed",String(button.dataset.quality===selectedQuality)));
  if(persist){try{localStorage.setItem(QUALITY_KEY,selectedQuality)}catch{}}
  createStars();
  createAmbientSpace();
...
function createStars() {
  ...
  const profile=QUALITY_PROFILES[resolvedQuality]||QUALITY_PROFILES.balanced;
  const deepRandom=seededRandom(STAR_LAYOUT_SEED+profile.deepStars);
  for(let i=0;i<profile.deepStars;i+=1){
    const star=document.createElement("span");
    star.className="star deep-star";
    star.style.left=`${deepRandom()*100}%`;
    star.style.top=`${deepRandom()*100}%`;
    star.style.setProperty("--size",`${(deepRandom()*1.9+.55).toFixed(2)}px`);
    star.style.setProperty("--base-opacity",(deepRandom()*.28+.06).toFixed(2));
    star.style.setProperty("--twinkle",`${(deepRandom()*6+4).toFixed(1)}s`);
  ...
  const exclusionZones=[
    {x:1800,y:1300,r:220},{x:1470,y:1110,r:95},{x:2140,y:1030,r:120},{x:1260,y:1540,r:190},
    {x:2400,y:1570,r:115},{x:920,y:720,r:190},{x:2820,y:830,r:220},{x:620,y:2050,r:165},
    {x:3080,y:2070,r:175},{x:1800,y:2350,r:150}
  ];
  const placed=[];
  ...
  for(let i=0;i<TOTAL_LIGHTS;i+=1){
    ...
    star.dataset.message=starMessages[i]||"Una parte de mí también llegó hasta esta luz.";
    star.style.left=`${x}px`;
    star.style.top=`${y}px`;
    star.style.setProperty("--size",`${(random()*3.4+2.1).toFixed(2)}px`);
    star.style.setProperty("--dormant-opacity",(random()*.1+.055).toFixed(2));
    star.setAttribute("aria-label",`Descubrir la luz ${i+1}`);
    star.addEventListener("pointerdown",e=>e.stopPropagation());
    star.addEventListener("click",e=>{e.stopPropagation();if(!state.movedDuringPointer)discoverStar(star)});
...
function createAmbientSpace() {
  ...
  const random=seededRandom(88731); const profile=QUALITY_PROFILES[resolvedQuality]||QUALITY_PROFILES.balanced; const moteCount=profile.motes; const bodyCount=profile.bodies;
...
function scheduleShootingStar(initial=false){clearTimeout(shootingStarTimer);const p=QUALITY_PROFILES[resolvedQuality]||QUALITY_PROFILES.balanced;const delay=initial?4600:p.shootMin+Math.random()*(p.shootMax-p.shootMin);shootingStarTimer=setTimeout(()=>{if(state.started&&!document.hidden&&!document.body.classList.contains("letter-open")){launchShootingStar()}scheduleShootingStar(false)},delay)}
...
function saveProgress(){
  try{
    localStorage.setItem(STORAGE_KEY,JSON.stringify({
      ...
      polarisUnlocked:state.polarisUnlocked,
...
function loadProgress(){
  try{
    ...
    state.visitedPlanets=new Set((data.visitedPlanets||[]).filter(x=>validPlanets.has(x)));
    state.visitedStars=new Set((data.visitedStars||[]).map(String).filter(x=>Number(x)>=0&&Number(x)<TOTAL_LIGHTS));
    state.secretUnlocked=state.visitedPlanets.size===8;
    state.polarisUnlocked=state.visitedStars.size===TOTAL_LIGHTS||Boolean(data.polarisUnlocked);
    state.letterOpened=Boolean(data.letterOpened);
    state.epilogueUnlocked=Boolean(data.epilogueUnlocked)||state.letterOpened;
  }catch(e){console.warn("No se pudo recuperar el progreso",e)}
}
function applySavedProgress(){
  document.querySelectorAll(".planet").forEach(p=>{if(state.visitedPlanets.has(p.dataset.planet)){p.classList.remove("dormant","awakened");p.classList.add("discovered")}});
  document.querySelectorAll(".interactive-star").forEach(s=>{if(state.visitedStars.has(s.dataset.starId)){s.classList.remove("dormant","awakened");s.classList.add("discovered")}});
  progressCount.textContent=state.visitedPlanets.size;
  starProgressCount.textContent=state.visitedStars.size;
  if(state.secretUnlocked)revealMoon(false);
  if(state.polarisUnlocked){revealPolaris(false);if(openLetterAgain)openLetterAgain.hidden=false}
  if(state.epilogueUnlocked&&openEpilogueButton)openEpilogueButton.hidden=false;
}
...
async function beginExperience(){if(state.started)return;state.started=true;initAudioContext();startSoundtrackFromGesture();originStar.disabled=true;originStar.style.pointerEvents="none";originStar.style.transition="opacity .65s ease, transform .65s cubic-bezier(.16,1,.3,1), filter .65s ease";originStar.style.opacity="0";originStar.style.transform="scale(.12)";originStar.style.filter="blur(6px)";setTimeout(()=>{originStar.style.visibility="hidden"},680);introInstruction.style.opacity="0";introText.classList.add("is-changing");await wait(700);introText.textContent="Nunca imaginé que aquella conversación terminaría convirtiéndose en el lugar donde más paz encontraría.";introText.classList.remove("is-changing");await wait(2500);intro.classList.add("is-leaving");viewport.classList.remove("is-locked");viewport.classList.add("is-ready");resetCamera();scheduleShootingStar(true);await wait(1900);await showNarrative("Los planetas guardan capítulos. Las luces pequeñas guardan pensamientos.",3400);showReturnToast()}
...
function getNextTrack(){if(!trackBag.length)refillTrackBag();return trackBag.shift()||tracks[0]}
...
function openLightMessage(message, options = {}){
  ...
  lightMessage.classList.toggle("final-light-message", type === "final-light");
  ...
  // La música continúa al mismo volumen mientras se lee.

  // La luz 80 permanece abierta hasta que Axel la cierre. Solo después aparece Polaris.
  if(duration > 0){
...
function closeLightMessage(){
  ...
  const wasFinalMessage=lightMessage.classList.contains("final-light-message");
  lightMessage.classList.remove("moon-message","final-light-message");

  if(state.pendingPolarisReveal||wasFinalMessage||(state.visitedStars.size===TOTAL_LIGHTS&&!secretObject?.classList.contains("is-revealed"))){
    state.pendingPolarisReveal=false;
    state.polarisUnlocked=true;
    saveProgress();
    setTimeout(()=>revealPolaris(true),550);
    return;
  }

  if(state.pendingEpilogue){
    state.pendingEpilogue=false;
    setTimeout(openEpilogue,700);
  }
}

function discoverStar(star){
  const id=star.dataset.starId;
  const isNewLight=!state.visitedStars.has(id);
  ...
  if(isNewLight){
    ...
    if(state.visitedStars.size===TOTAL_LIGHTS){
      state.polarisUnlocked=true;
      state.pendingEpilogue=false;
      state.pendingPolarisReveal=true;
      openEpilogueButton.hidden=true;
      playUnlockSfx();
      saveProgress();

      openLightMessage(
        "Estas nunca fueron solo estrellas ni luces. Cada una era un pensamiento mío que encontró una forma de quedarse cerca de ti. Y aun después de ochenta, siguen sin ser suficientes para contener todo lo que pienso y siento por ti.",
        {
          type:"final-light",
          label:"La luz número ochenta",
          closeText:"Toca para descubrir la luz que aún faltaba",
          duration:0
        }
      );
      return;
...
function revealPolaris(center=true){
  if(!secretObject||!world)return;
  const x=Number(secretObject.dataset.x)||1800;
  const y=Number(secretObject.dataset.y)||2350;
  if(secretObject.parentElement!==world)world.appendChild(secretObject);
  secretObject.style.left=`${x}px`;
  secretObject.style.top=`${y}px`;
  secretObject.style.display="block";
  secretObject.style.visibility="visible";
  secretObject.style.opacity="1";
  secretObject.style.pointerEvents="auto";
  secretObject.classList.remove("hidden-object");
  secretObject.classList.add("is-revealed","polaris-final");
  secretObject.setAttribute("aria-hidden","false");
  secretObject.setAttribute("aria-label","Abrir la carta de Polaris");
  const label=secretObject.querySelector(".planet-label");
  if(label)label.textContent="Polaris";
  state.polarisUnlocked=true;
  if(openLetterAgain)openLetterAgain.hidden=false;
  saveProgress();

  if(center){
    secretObject.classList.remove("polaris-arrival");
    requestAnimationFrame(()=>requestAnimationFrame(()=>secretObject.classList.add("polaris-arrival")));
    centerOnWorldPoint(x,y,window.innerWidth<700?1.18:1.2,true);
    setTimeout(()=>{
      returnToast.textContent="Polaris apareció. Tócala para abrir la carta.";
      returnToast.classList.add("is-visible");
      clearTimeout(returnToastTimer);
      returnToastTimer=setTimeout(()=>returnToast.classList.remove("is-visible"),5200);
    },900);
  }
}

function openLetter(){if(!state.polarisUnlocked)return;closeMenu();state.letterOpened=true;saveProgress();letterScene.classList.add("is-open");letterScene.setAttribute("aria-hidden","false");document.body.classList.add("letter-open")}
function closeLetterScene(){letterScene.classList.remove("is-open");letterScene.setAttribute("aria-hidden","true");document.body.classList.remove("letter-open");state.epilogueUnlocked=true;if(openEpilogueButton)openEpilogueButton.hidden=false;saveProgress();if(!state.finalLineShown){state.finalLineShown=true;setTimeout(()=>showNarrative("Este universo siempre será tuyo.",3600),900)}}
function openEpilogue(){if(!state.epilogueUnlocked)return;closeMenu();epilogueScene.classList.add("is-open");epilogueScene.setAttribute("aria-hidden","false");document.body.classList.add("epilogue-open")}
function closeEpilogueScene(){epilogueScene.classList.remove("is-open");epilogueScene.setAttribute("aria-hidden","true");document.body.classList.remove("epilogue-open")}
...
function guideToLight(){const remaining=[...document.querySelectorAll(".interactive-star:not(.discovered)")];if(!remaining.length){closeMenu();if(state.polarisUnlocked){revealPolaris(false);centerOnWorldPoint(Number(secretObject.dataset.x)||1800,Number(secretObject.dataset.y)||2350,window.innerWidth<700?1.08:1.2,true)}return}const star=remaining[Math.floor(Math.random()*remaining.length)];closeMenu();centerOnWorldPoint(Number(star.dataset.x),Number(star.dataset.y),Math.max(camera.scale,.88),true);star.classList.add("guided-light");setTimeout(()=>star.classList.remove("guided-light"),6000)}
function resetAllProgress(){if(!confirm("¿Reiniciar todos los planetas, luces y la carta?"))return;try{localStorage.removeItem(STORAGE_KEY)}catch{}location.reload()}

function setStartedForTest(){
  if(state.started)return;
  state.started=true;
  intro.classList.add("is-leaving");
  originStar.style.visibility="hidden";
  viewport.classList.remove("is-locked");
  viewport.classList.add("is-ready");
  scheduleShootingStar(true);
}
function syncStarClasses(){
  document.querySelectorAll(".interactive-star").forEach(star=>{
    const found=state.visitedStars.has(star.dataset.starId);
    star.classList.toggle("discovered",found);
    star.classList.toggle("dormant",!found);
    star.classList.remove("awakened","guided-light");
  });
  starProgressCount.textContent=state.visitedStars.size;
}
function prepareLight80Test(){
  setStartedForTest();
  closeMenu();
  clearTimeout(lightMessageTimer);
  lightMessage.classList.remove("is-open","moon-message","final-light-message");
  state.pendingPolarisReveal=false;
  state.visitedStars=new Set(Array.from({length:TOTAL_LIGHTS-1},(_,i)=>String(i)));
  state.polarisUnlocked=false;
  state.pendingPolarisReveal=false;
  state.letterOpened=false;
  secretObject?.classList.remove("is-revealed","polaris-final","polaris-arrival");
  secretObject?.classList.add("hidden-object");
  if(secretObject){secretObject.style.opacity="";secretObject.style.visibility="";secretObject.style.pointerEvents="";secretObject.setAttribute("aria-hidden","true")}
  syncStarClasses();
  saveProgress();
  const last=document.querySelector(`.interactive-star[data-star-id="${TOTAL_LIGHTS-1}"]`);
  if(last){centerOnWorldPoint(Number(last.dataset.x),Number(last.dataset.y),window.innerWidth<700?.92:1.05,true);last.classList.add("guided-light");setTimeout(()=>last.classList.remove("guided-light"),7000)}
  setTimeout(()=>showNarrative("Modo de prueba: toca la única luz que queda.",2200),400);
}
function previewFinalMessageTest(){
  setStartedForTest();
  state.visitedStars=new Set(Array.from({length:TOTAL_LIGHTS},(_,i)=>String(i)));
  state.polarisUnlocked=true;
  state.pendingPolarisReveal=true;
  syncStarClasses();
  saveProgress();
  closeMenu();
  openLightMessage("Estas nunca fueron solo estrellas ni luces. Cada una era un pensamiento mío que encontró una forma de quedarse cerca de ti. Y aun después de ochenta, siguen sin ser suficientes para contener todo lo que pienso y siento por ti.",{type:"final-light",label:"La luz número ochenta",closeText:"Toca para descubrir la luz que aún faltaba",duration:0});
}
function previewPolarisTest(){
  setStartedForTest();
  state.visitedStars=new Set(Array.from({length:TOTAL_LIGHTS},(_,i)=>String(i)));
  state.polarisUnlocked=true;
  syncStarClasses();
...
function installTestTools(){
  ...
  const actions=menuPanel?.querySelector(".menu-actions");
  if(!actions)return;
  const box=document.createElement("div");
  box.className="test-tools";
  box.innerHTML='<p>Modo de prueba</p>';
  const add=(text,handler)=>{const button=document.createElement("button");button.type="button";button.textContent=text;button.addEventListener("click",handler);box.appendChild(button)};
  add("Preparar la luz 80",prepareLight80Test);
  add("Ver mensaje de la luz 80",previewFinalMessageTest);
  add("Mostrar Polaris",previewPolarisTest);
  add("Abrir la carta",()=>{state.polarisUnlocked=true;openLetter()});
  actions.before(box);
}
...
if(secretObject){secretObject.addEventListener("pointerdown",e=>e.stopPropagation());secretObject.addEventListener("click",e=>{e.stopPropagation();if(!state.movedDuringPointer)openLetter()})}
...
toggleAudio.addEventListener("click",toggleSound);nextTrackButton.addEventListener("click",nextTrack);qualityButtons.forEach(button=>button.addEventListener("click",()=>applyQuality(button.dataset.quality,true)));resetView.addEventListener("click",()=>{closeMenu();resetCamera()});findLight.addEventListener("click",guideToLight);openLetterAgain.addEventListener("click",openLetter);openEpilogueButton.addEventListener("click",openEpilogue);resetProgress.addEventListener("click",resetAllProgress);
...
if(secretObject){
  secretObject.style.left=`${secretObject.dataset.x||1800}px`;
  secretObject.style.top=`${secretObject.dataset.y||2350}px`;
}
window.addEventListener("resize",()=>{if(selectedQuality==="auto")applyQuality("auto",false);else{scheduleRevealCheck();createAmbientSpace()}});
document.addEventListener("visibilitychange",()=>{if(!document.hidden&&state.started)scheduleShootingStar(true)});

probeTracks();applyQuality(selectedQuality,false);loadProgress();applySavedProgress();installTestTools();applyCamera(false);introSequence();
